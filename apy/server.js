...
app.use(
    express.static(path.join(__dirname, serverConfig.staticPath), {
...
app.get("/run-check", async (req, res) => {
    ...
    try {
        ...
        // Log request info
        log.info(
            `APY check requested. Force refresh: ${forceRefresh}, Last run: ${
                cacheInfo.lastRun ? cacheInfo.lastRun.toISOString() : "never"
            }`
        );
